<?php
	...
	if($name!='' && $phone!='')
	{
		...
		$postFields = array(
			...
			// 'auth_token' => '...',
			'source' => FRAPPE_ENQUIRY_SOURCE_TEST_BOOKING,
		        'name' => $name,
		        'email' => $email,
		        'phone' => $phone,
		        'age' => $age,
		...
		if (!empty($_FILES['pre_file']['name']) && !empty($_FILES['pre_file']['tmp_name']) && is_uploaded_file($_FILES['pre_file']['tmp_name'])) {
		        $tmp = $_FILES['pre_file']['tmp_name'];
		        $mime = function_exists('mime_content_type') ? mime_content_type($tmp) : 'application/octet-stream';
		        $postFields['pre_file'] = new CURLFile($tmp, $mime, $_FILES['pre_file']['name']);
		}

		error_log('Frappe website webhook: start booking_id=' . (string) (int) $booking_id);
		...
		if ($frappeErr || $frappeHttp !== 200) {
		        error_log('Frappe website webhook: HTTP ' . $frappeHttp . ' curl_err=' . $frappeErr . ' body=' . substr((string) $frappeBody, 0, 800));
		} else {
	        	$fj = json_decode($frappeBody, true);
		        if (!is_array($fj) || ($fj['message'] ?? '') !== 'ok') {
        	        	error_log('Frappe website webhook unexpected JSON: ' . substr((string) $frappeBody, 0, 800));
        		} else {
        		        error_log('Frappe website webhook: success message=ok booking_id=' . (string) (int) $booking_id);
        		}
		}
		echo "0";
		exit;
